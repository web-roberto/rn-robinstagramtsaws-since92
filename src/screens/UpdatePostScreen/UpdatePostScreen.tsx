...
const UpdatePostScreen = () => {
    ...
    useEffect(() => {
        if (updateData){navigation.goBack()}
      }, [updateData,navigation])
    
    const submit=async ()=>{
        if (!post) {return;}
        doUpdatePost({variables:{input:{id:post.id,description,_version:post._version}}})
    }     


    if (loading) {return <ActivityIndicator/>}
    if (error||updateError) {return <ApiErrorMessage title='Failed to fetch or update the post' message={error?.message||updateError?.message}/>}
